...
export async function GET(
  ...
  try {
    ...
    console.log('📊 Loading stats for tenant:', subdomain);

    // First, resolve subdomain to tenant ID if needed
    let actualTenantId = subdomain;

    // Try to get tenant by subdomain first (if subdomain is actually a subdomain)
    ...
    if (!tenant) {
      console.log('❌ Tenant not found for:', subdomain);
